package fr.plb.ecom_product.product.infrastructure.primary;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import fr.plb.ecom_product.product.application.ProductsApplicationService;
import fr.plb.ecom_product.product.domain.aggregate.Product;
import fr.plb.ecom_product.product.domain.vo.OrderProductQuantity;
import fr.plb.ecom_product.product.domain.vo.PublicId;
import fr.plb.ecom_product.product.infrastructure.primary.exceptions.MultipartPictureException;
...
public class ProductsAdminResource {
    ...
    @GetMapping("/by-public-ids")
    @PreAuthorize("hasAnyRole('" + ROLE_ADMIN + "')")
    public ResponseEntity<List<RestProduct>> getProductsByPublicIdsIn(@RequestParam("publicIds") String publicIds) {
        List<PublicId> split = Arrays.stream(publicIds.split(","))
                .map(publicIdsString -> new PublicId(UUID.fromString(publicIdsString))).toList();
        List<Product> productsByPublicIdsIn = productsApplicationService.getProductsByPublicIdsIn(split);
//        return ResponseEntity.internalServerError().build();
        return ResponseEntity.ok(productsByPublicIdsIn.stream().map(RestProduct::fromDomain).toList());
    }

    @PutMapping("/update-product-quantity")
    @PreAuthorize("hasAnyRole('" + ROLE_ADMIN + "')")
    public ResponseEntity<Void> updateProductQuantity(List<OrderProductQuantity> orderProductQuantities) {
        productsApplicationService.updateProductQuantity(orderProductQuantities);
